...
"border-color: rgb(121, 122, 126);")
        ...
        font = QtGui.QFont()
        font.setFamily("?úÏª¥ ÎßêÎûëÎßêÎûë Regular")
        ...
        self.title_1.setGeometry(QtCore.QRect(100, 150, 741, 80)) # ?úÏûë?ÑÏπò x,y / ?àÎπÑ,?íÏù¥
        self.title_2.setGeometry(QtCore.QRect(100, 230, 741, 100)) # ?úÏûë?ÑÏπò x,y / ?àÎπÑ,?íÏù¥
        font_1 = QtGui.QFont()
        font_2 = QtGui.QFont()
...
"background-color: qlineargradient(spread:pad, x1:0, y1:0, x2:0, y2:0, stop:0 rgba(0, 0, 0, 0), stop:1 rgba(255, 255, 255, 0));")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.next_button.setText(_translate("MainWindow", "Click"))

    def progressbar_change(self):
        while self.Mainwindow.text() == "Mainwindow":
...
class secondwindow(QDialog, QFrame, form_secondwindow):
    ...
    def filedialog_open(self):
        ...
        if fname[0]:
            ...
            self.pixmap = self.pixmap.scaled(481,621) # ?¥Î?ÏßÄ ?§Ï???Î≥Ä??
            self.image_frame.move(50,50) # ?úÏûë?ÑÏπò
            self.image_frame.setPixmap(self.pixmap)  # ?¥Î?ÏßÄ ?∏ÌåÖ
            self.image_frame.setContentsMargins(0,0,0,0)
            self.image_frame.resize(481,621)  # ?ÑÎ†à???§Ï???
    ...
    # okÎ≤ÑÌäº ?¥Î¶≠??third page Î°??çÏä§???òÍ∏∞Í∏?
    ...
    def gotonextpage(self):
        if se.ok_button.text() != "Insert":
            try:
                ...
                th.yolo_frame.setPixmap(th.yolo_img)  # ?¥Î?ÏßÄ ?∏ÌåÖ
                th.yolo_frame.setContentsMargins(0,0,0,0)
                th.yolo_frame.resize(411,521)  # ?ÑÎ†à???§Ï???
